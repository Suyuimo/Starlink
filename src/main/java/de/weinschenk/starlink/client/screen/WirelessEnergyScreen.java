...
public class WirelessEnergyScreen extends AbstractContainerScreen<WirelessEnergyMenu> {

    private EditBox channelBox;

    public WirelessEnergyScreen(WirelessEnergyMenu menu, Inventory inv, Component title) {
    ...
    protected void init() {
        super.init();
        channelBox = new EditBox(font, leftPos + 68, topPos + 94, 100, 12, Component.literal("Kanal"));
    ...
    protected void renderBg(GuiGraphics g, float pt, int mx, int my) {
        ...
        // Energy bar  (left side, x=12, y=16, w=18, h=58)
        drawBar(g, x + 12, y + 16, 18, 58, menu.getEnergy(), menu.getMaxEnergy(), 0xFFFF6A00);

        // Satellite bar  (x=38, y=16, w=18, h=58)
        drawBar(g, x + 38, y + 16, 18, 58, menu.getSatCount(), 20, 0xFF00AAFF);

        // Channel row separator
        g.fill(x + 1, y + 90, x + imageWidth - 1, y + 91, 0xFF333355);
    }
    ...
    protected void renderLabels(GuiGraphics g, int mx, int my) {
        ...
        g.drawString(font, "RF",  13, 76, 0xFF6A00, false);
        g.drawString(font, "SAT", 37, 76, 0x00AAFF, false);

        int e = menu.getEnergy(), me = menu.getMaxEnergy();
        g.drawString(font, formatRF(e) + " / " + formatRF(me), 68, 20, 0xFFFFFF, false);
        g.drawString(font, "Satelliten: " + menu.getSatCount(), 68, 32, 0x88CCFF, false);

        if (menu.isReceiver()) {
            Component link = menu.isLinked()
                    ? Component.literal("\u00a7aVerknüpft")
                    : Component.literal("\u00a7cNicht verknüpft");
            g.drawString(font, link, 68, 44, 0xFFFFFF, false);
        }

        // Channel label
        int channelColor = menu.isPrivate() ? 0xFFAA44 : 0x88AA88;
        g.drawString(font, "Kanal:", 68, 84, channelColor, false);
    }
