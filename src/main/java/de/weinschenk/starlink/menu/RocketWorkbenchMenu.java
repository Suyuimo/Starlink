...
import de.weinschenk.starlink.recipe.RocketCraftingRecipe;
...
public class RocketWorkbenchMenu extends AbstractContainerMenu {
    ...
    public RocketWorkbenchMenu(int containerId, Inventory inv) {
        this(containerId, inv, ContainerLevelAccess.NULL);
    }

    public RocketWorkbenchMenu(int containerId, Inventory inv, ContainerLevelAccess access) {
    ...
    private void updateResult(Level level) {
        ...
        Optional<RocketCraftingRecipe> optional = level.getServer().getRecipeManager()
                .getRecipeFor(ModRecipeTypes.ROCKET_CRAFTING.get(), craftSlots, level);

        if (optional.isPresent()) {
            RocketCraftingRecipe recipe = optional.get();
    ...
    public boolean stillValid(Player player) {
        return stillValid(access, player, ModBlocks.ROCKET_WORKBENCH.get());
    }
